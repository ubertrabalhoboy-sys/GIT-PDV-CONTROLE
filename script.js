...
document.addEventListener('DOMContentLoaded', () => {
   ...
   const renderViewContent = (viewId, viewContainer) => {
        ...
        // Remove listeners da view anterior para evitar duplicação
        if (currentViewListenerController) {
            currentViewListenerController.abort();
        }
        currentViewListenerController = new AbortController();
        const { signal } = currentViewListenerController;

        // Chama a função para desenhar os dados da view
        switch (viewId) {
        ...
        // Adiciona os listeners de eventos para a view atual
        // Isso garante que eles sejam adicionados apenas UMA VEZ.
        switch (viewId) {
            case 'produtos':
                setupProdutosListeners(viewContainer, signal);
                break;
            case 'clientes':
                setupClientesListeners(viewContainer, signal);
                break;
        }
    ...
    // NOVO: Listener para a tela de Produtos
    function setupProdutosListeners(view, signal) {
    ...
    // NOVO: Listener para a tela de Clientes
    function setupClientesListeners(view, signal) {
        const form = view.querySelector('#add-client-form');
        const tableBody = view.querySelector('#clients-table-body');
        const searchInput = view.querySelector('#client-search');
        let currentEditingId = null;
        ...
        form.addEventListener('reset', () => setTimeout(resetForm, 0), { signal });
        ...
        tableBody.addEventListener('click', async (e) => {
            ...
            if (btn.classList.contains('remove-client-btn')) {
                ...
                if(client) {
                 ...
                 // A lógica do modal de detalhes continua a mesma
            }
        }, { signal });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderViewContent = (viewId) => {
        ...
        switch (viewId) {
            ...
            case 'clientes':
                ...
                const form = viewContainer.querySelector('#add-client-form');
                const searchInput = viewContainer.querySelector('#client-search');
                let currentEditingId = null;

                const resetForm = () => {
                    form.reset();
                    currentEditingId = null;
                    form.querySelector('#client-form-title').textContent = 'Adicionar Novo Cliente';
                    form.querySelector('#client-form-btn-text').textContent = 'Salvar Cliente';
                    form.querySelector('#client-form-cancel').classList.add('hidden');
                };
                ...
                searchInput.addEventListener('input', () => {
                    const term = searchInput.value.toLowerCase();
                    const filtered = state.db.clients.filter(c => c.name.toLowerCase().includes(term) || (c.phone && c.phone.includes(term)));
                ...
                viewContainer.addEventListener('click', async (e) => {
                    ...
                    if (btn.classList.contains('edit-client-btn')) {
                        ...
                        if (client) {
                            currentEditingId = client.id;
                            form.querySelector('#client-form-name').value = client.name;
                            form.querySelector('#client-form-phone').value = client.phone || '';
                            form.querySelector('#client-form-email').value = client.email || '';
                            form.querySelector('#client-form-address').value = client.address || '';
                            form.querySelector('#client-form-title').textContent = 'Editando Cliente';
                            form.querySelector('#client-form-btn-text').textContent = 'Atualizar';
                            form.querySelector('#client-form-cancel').classList.remove('hidden');
                            form.querySelector('#client-form-name').focus();
                        }
                    }
                    if (btn.classList.contains('remove-client-btn')) {
                        showConfirmModal('Deseja remover este cliente?', async () => {
                            await deleteDoc(doc(db, "clients", clientId));
    ...
    // NOVO: Função inteira para renderizar a tela de Clientes (SIMPLIFICADA)
    ...
    function renderProdutos() {
        ...
        // A função agora só tem a lógica para desenhar a tabela
        tableBody.innerHTML = '';
        if (state.db.products.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="4" class="text-center p-8 text-slate-500">Nenhum produto cadastrado.</td></tr>`;
            return;
