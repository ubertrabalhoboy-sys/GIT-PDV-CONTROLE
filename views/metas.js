...
export function renderMetas() {
    ...
    const updateGoalsUI = (sales) => {
        ...
        for (let i = 0; i < 365; i++) {
            ...
            if (daySales >= (goals.daily || 0.01)) {
                ...
                if (i > 0) { // Don't break on the first day (today) if there are no sales yet
                    break;
                }
            }
    ...
    const q = query(collection(db, "sales"), where("vendedor", "==", state.loggedInUser.name), where("storeId", "==", state.selectedStore.id));
    
    // O listener é anexado aqui e gerenciado pelo main.js
