...
import { addDoc, setDoc, deleteDoc, doc, query, where, getDocs, collection } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
export function renderClientes(state) {
        ...
        sortedClients.forEach(client => {
            ...
            <tr class="bg-white/50 dark:bg-slate-900/50 border-b border-slate-300 dark:border-slate-800 hover:bg-slate-200/50 dark:hover:bg-slate-800/50">
                ...
                <td class="px-6 py-4 text-center space-x-2">
                    ...
                    <button data-client-id="${client.id}" class="edit-client-btn text-amber-500 hover:text-amber-700" title="Editar"><i data-lucide="edit" class="w-4 h-4 pointer-events-none"></i></button>
    ...
    tableBody.addEventListener('click', async (e) => {
        ...
        if (btn.classList.contains('remove-client-btn')) {
            ...
            try { await deleteDoc(doc(db, "clients", clientId)); showToast('Cliente removido!', 'success'); }
            ...
            // Mostrar modal de detalhes do cliente com histórico (consulta sales)
            const client = state.db.clients.find(c => c.id === clientId);
            if (!client) return;
            const salesQuery = query(collection(db, "sales"), where("clientId", "==", clientId));
            const salesSnapshot = await getDocs(salesQuery);
            const clientSales = salesSnapshot.docs.map(d => d.data());
