...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderViewContent = (viewId) => {
        ...
        switch (viewId) {
            ...
            default:
                viewContainer.innerHTML = `<p class="p-6">View não encontrada.</p>`;
        }
    };
    ...
    function renderClientes() {
        ...
        const paginationControls = view.querySelector('#clients-pagination-controls');
        ...
        const renderClientsTable = () => {
            clientCountEl.textContent = `${currentClientList.length} cliente(s) encontrado(s).`;
            
            const totalPages = Math.ceil(currentClientList.length / itemsPerPage);
            currentPage = Math.min(currentPage, totalPages) || 1;

            const start = (currentPage - 1) * itemsPerPage;
            const end = start + itemsPerPage;
            const paginatedClients = currentClientList.slice(start, end);
            ...
            const rowsHTML = paginatedClients.map(client => `
            ...
            paginationControls.innerHTML = `
                <button id="prev-page-clients" class="px-3 py-1 rounded-md bg-slate-200 dark:bg-slate-700 disabled:opacity-50" ${currentPage === 1 ? 'disabled' : ''}>Anterior</button>
                <span>Página ${currentPage} de ${totalPages}</span>
                <button id="next-page-clients" class="px-3 py-1 rounded-md bg-slate-200 dark:bg-slate-700 disabled:opacity-50" ${currentPage === totalPages ? 'disabled' : ''}>Próxima</button>
            `;

            document.getElementById('prev-page-clients')?.addEventListener('click', () => {
                if (currentPage > 1) {
                    currentPage--;
                    renderClientsTable();
                }
            ...
            document.getElementById('next-page-clients')?.addEventListener('click', () => {
        ...
        const applyFilters = () => {
            const sortedClients = [...state.db.clients].sort((a, b) => a.name.localeCompare(b.name));
            currentClientList = sortedClients;
            currentPage = 1;
            renderClientsTable();
        };
        ...
        searchInput.addEventListener('input', () => {
            ...
            currentClientList = state.db.clients.filter(c =>
        ...
        clientFilterForm.addEventListener('submit', async (e) => {
            ...
            currentClientList = state.db.clients.filter(client => clientIdsFromSales.includes(client.id)).sort((a, b) => a.name.localeCompare(b.name));
            currentPage = 1;
            renderClientsTable();
        });
...
function renderProdutos() {
    ...
    const renderProductsPage = () => {
        ...
        <div class="p-4 sm:p-6 space-y-6">
            ...
            <div class="custom-card rounded-lg">
                <div class="overflow-x-auto">
                 ...
                 <div id="products-pagination-controls" class="p-4 flex justify-between items-center text-sm"></div>
            </div>
        </div>
        `;
        ...
        const paginationControls = view.querySelector('#products-pagination-controls');
        ...
        const renderProductsTable = () => {
            ...
            paginationControls.innerHTML = `
                <button id="prev-page-products" class="px-3 py-1 rounded-md bg-slate-200 dark:bg-slate-700 disabled:opacity-50" ${currentPage === 1 ? 'disabled' : ''}>Anterior</button>
                <span>Página ${currentPage} de ${totalPages}</span>
                <button id="next-page-products" class="px-3 py-1 rounded-md bg-slate-200 dark:bg-slate-700 disabled:opacity-50" ${currentPage === totalPages ? 'disabled' : ''}>Próxima</button>
            `;

            document.getElementById('prev-page-products')?.addEventListener('click', () => {
                if (currentPage > 1) {
                    currentPage--;
                    renderProductsTable();
                }
            ...
            document.getElementById('next-page-products')?.addEventListener('click', () => {
                if (currentPage < totalPages) {
                    currentPage++;
                    renderProductsTable();
                }
    ...
    function renderPedidos() {
        ...
        const renderTable = () => {
            ...
            const paginationControls = c.querySelector('#orders-pagination-controls');
            
            const totalPages = Math.ceil(currentSalesList.length / itemsPerPage);
            currentPage = Math.min(currentPage, totalPages) || 1;
            
            const start = (currentPage - 1) * itemsPerPage;
            const end = start + itemsPerPage;
            const paginatedSales = currentSalesList.slice(start, end);
            ...
            const rowsHTML = paginatedSales.map(s => {
            ...
            paginationControls.innerHTML = `
                <button id="prev-page-orders" class="px-3 py-1 rounded-md bg-slate-200 dark:bg-slate-700 disabled:opacity-50" ${currentPage === 1 ? 'disabled' : ''}>Anterior</button>
                <span>Página ${currentPage} de ${totalPages}</span>
                <button id="next-page-orders" class="px-3 py-1 rounded-md bg-slate-200 dark:bg-slate-700 disabled:opacity-50" ${currentPage === totalPages ? 'disabled' : ''}>Próxima</button>
            `;

            document.getElementById('prev-page-orders')?.addEventListener('click', () => {
                if (currentPage > 1) {
                    currentPage--;
                    renderTable();
                }
            ...
            document.getElementById('next-page-orders')?.addEventListener('click', () => {
                if (currentPage < totalPages) {
                    currentPage++;
                    renderTable();
                }
    ...
    function renderRanking() {
        ...
        const updateRankingUI = (sales, period) => {
            ...
            podiumContainer.innerHTML = `<div class="flex flex-col sm:flex-row justify-center items-end gap-4 sm:gap-2 md:gap-4">${podiumHTML}</div>`;

            if(others.length > 0) {
                const listHTML = others.map((seller, index) => `
                    <li class="flex items-center justify-between p-3 bg-slate-200/50 dark:bg-slate-800/50 rounded-lg">
                        <div class="flex items-center gap-3">
                            <span class="font-bold text-slate-500 dark:text-slate-400 w-6 text-center">${index + 4}</span>
                ...
                `).join('');
                listContainer.innerHTML = `<ul class="space-y-2">${listHTML}</ul>`;
            } else {
                 listContainer.innerHTML = '';
            }
            window.lucide.createIcons();
    ...
    function renderConfiguracoes() {
        ...
        if (state.loggedInUser.role === 'superadmin') {
            ...
            const renderStoresList = () => {
                const storesHTML = state.db.stores.map(store => `
                    <li class="flex justify-between items-center bg-slate-100 dark:bg-slate-700 p-2 rounded-md">
                ...
                `).join('');
                storesListEl.innerHTML = storesHTML;
                window.lucide.createIcons();
            };
